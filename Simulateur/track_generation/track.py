...
class Grid:
    ...
    def __init__(self, tile_definitions, global_params):
        ...
        # Find the next track number to save
        self.track_number = self.find_next_track_number()
    ...
    def find_next_track_number(self):
        """
        Finds the next available track number based on existing trackN.png files.
        """
        track_files = [f for f in os.listdir("../tracks") if f.startswith("track") and f.endswith(".png")]
        track_numbers = []

        for f in track_files:
            try:
                num = int(f.replace("track", "").replace(".png", ""))
                track_numbers.append(num)
            except ValueError:
                continue  # Ignore files that don't fit the trackN.png pattern
        
        if track_numbers:
            return max(track_numbers) + 1  # Next track number
        return 0  # Start from track0.png if no track files exist    
    
    def on_key(self, event):
        """
        Handles key events:
        - 'a': Save the track with the next available number
        - 'n': Generate new track
        - 'c': Reset the track numbering
        """
        if event.key == 'a':  # Save the track
            # self.track_number = self.find_next_track_number()
            filename = f"../tracks/track{self.track_number}.png"
            plt.imsave(filename, self.bitmap, cmap='Greys')
            print(f"Track saved as '{filename}'")
            self.track_number += 1  # Increment track number for next save
            plt.close(self.fig)
        elif event.key == 'n':  # Generate new track
            plt.close(self.fig)  # Close the figure and generate a new one
        elif event.key == 'c': # close the program 
            plt.close(self.fig)
            exit()
