...
align_steer = CTkButton(master=steer_frame, text="align", width=72, height=24, command=align_command)
...
set_entry(min_steer, parameters["steer"] ["min"])
...
set_entry(max_steer, parameters["steer"] ["max"])
max_steer.bind("<KeyRelease>", lambda event: changed_controllers("steer"))

slider_steer = CTkSlider(master=steer_frame, command=lambda event: changed_controllers("steer"))
...
set_entry(min_speed, parameters["speed"] ["min"])
min_speed.bind(
...
set_entry(max_speed, parameters["speed"] ["max"])
max_speed.bind(
    "<KeyRelease>", lambda event: changed_controllers("speed"))
